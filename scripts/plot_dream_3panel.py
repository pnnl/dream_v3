...
Plots DREAM optimial sensor campaigns on 3-panel plume map
...
# python plot_dream_3panel.py ensemble_solutionSpace.txt best_campaigns.csv 
...
if (len(sys.argv) != 3):
    print ('Usage : plot_dream_3panel.py <xyz dat file>  <sensor best campaign file>\n')
    exit()
...
#bcfile='C:\\Users\\whit162\\OneDrive - PNNL\\Documents\\Projects\\DreamProject\\Results_20191205155836\\best_campaigns.csv'
if not re.search('.txt$',fname.lower()):
    print ('\nError - input file must be a *.txt file\n')
    exit()

# Determine the directory where we save results
new_folder = os.path.join(os.path.dirname(fname) + '/best_campaign_visualizations')
print(new_folder)
...
# Read in the best campaigns to df3
df3 = pd.DataFrame(columns=['campaign','cost','vol_aq_degraded','sensor_type','x','y','z','x2','y2'])
...
df3=df3.sort_values(by=['campaign','sensor_type'])
...
legendlabelcolor = "#666666" #Color of the labels for the legend (File:, Campaign:, etc). Original "#666666"
...
#### Now we create a map for each campaign ####
...
for campaign, group in df3.groupby(['campaign']):
    ...
    fignamepng='%s/campaign%03d.png'%(new_folder, campaign)
    print ('\nCreating figure: %s'%fignamepng)
    ...
    fig.text(0.74,textbottom,'%s\n%03d (zoomed view)\n%s\n$%s'%(ensemble,campaign,vad,cost),horizontalalignment='left', color=legendvaluecolor, fontsize=legendfontsize, **universalfont)
    
    fig.savefig(fignamepng)
    plt.close()

t1=float(time.process_time())
print ('\n\n  Done!![%4ds]   open %s/campaign*.*.png\n\n'%(t1-t0,new_folder))
