...
/**
 * Plots displayed for the user if they enable "show plots" on the last page. Show how campaigns are performing over time.
 * @author port091
 */
...
public class TimeToDetectionPlots extends JFrame {
	...
	private void initializeUI() {
		...
		// First plot
		ChartPanel chartPanel25 = new ChartPanel(createChart(newMoreThan90, "Iteration", "Time to detection (years)", "New campaign TTD (detecting scenarios only)"));
	...
	public void addData(int iteration, Campaign campaign, ScenarioSet set) {
				
		float totalTTDTriggerOnly = campaign.getObjectiveValue(OBJECTIVE.TTD, false, null);
		float detected = campaign.getObjectiveValue(OBJECTIVE.SCENARIOS_DETECTED, false, null);
		
		((DefaultCategoryDataset)newMoreThan90.getUnderlyingDataset()).addValue(totalTTDTriggerOnly, "new", String.valueOf(iteration));
		...
		((DefaultCategoryDataset)scenariosDetected.getUnderlyingDataset()).addValue(detected, "new", String.valueOf(iteration));
